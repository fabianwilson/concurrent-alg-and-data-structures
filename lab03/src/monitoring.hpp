...
char const* operator_name(StackOperator op) {
    switch (op) {
        case StackOperator::StackPush:
            return "push";
        case StackOperator::StackPop:
            return "pop";
        case StackOperator::StackSize:
            return "size";
        default:
            return "[UNKNOWN]";
    }
...
struct Operation {
    ...
    void print() {
        if (this->argument == NO_ARGUMENT_VALUE) {
            std::cout << operator_name(this->op) << "()";
        } else {
...
bool test_event(DS* data_structure, Event<Op>* event) {
    ...
    if (result != event->output) {
        std::cout << "- ";
        event->print();
        std::cout << " is invalid, since " << result << " was expected." << std::endl;
        std::cout << "  - Test State: ";
...
public:
    ...
    void add(Event<Op> event) {
        int output = event.output;
        this->reserve(event)->complete(output);
    }
